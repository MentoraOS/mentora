---
doc: canon-publication-reports
title: Rapports émis
type: publication
titre: canon
statut: "Projeté — R2-Projections Lot 5C (Compliance Package)"
corpus_ref: "foundation-v1.0.0 (8d095ee)"
proprietaire: Conseil Constitutionnel de Mentora
cree_par: "R2-Corpus Lot 1"
projete_par: "R2-Projections Lot 5C"
---

# Rapports

Contenant des rapports **émis** et des rapports de conformité. **Compliance
Package** — les six rapports (un rapport démontre, il ne décide jamais — N°46) :

| # | Rapport | Statut |
|---|---------|--------|
| 01 | [Publication Validation Report](01-publication-validation-report.md) | **CONFORME** |
| 02 | [Integrity Report](02-integrity-report.md) | **CONFORME** (42/42 empreintes) |
| 03 | [Coverage Report](03-coverage-report.md) | Source 100 %, Projection 100 % |
| 04 | [Traceability Report](04-traceability-report.md) | **CONFORME** (chaîne complète) |
| 05 | [Projection Consistency Report](05-projection-consistency-report.md) | **CONFORME** (dettes gouvernées CMP-0001/0002) |
| 06 | [Source Consistency Report](06-source-consistency-report.md) | **CONFORME** (cohérence F1→F5 acquise, F5.99) |

> Réservé aussi au **Production Closure Report (PCR-001)**, `ÉMIS`, signé par le
> CTO (acte accompli hors R2-Corpus). Les rapports **constatent**, ne corrigent
> jamais (N°49) ; chaque anomalie porte une identité permanente `CMP-NNNN` (N°48).
