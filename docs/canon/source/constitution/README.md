---
doc: canon-source-f2
title: F2 — Constitution (Source)
type: source
titre: constitution
statut: "R2-Corpus Lot 3 — reconstruction COMPLÈTE (F2.1 → F2.9 : 100 %)"
corpus_ref: "foundation-v1.0.0 (8d095ee)"
proprietaire: Conseil Constitutionnel de Mentora
cree_par: "R2-Corpus Lot 1"
---

# F2 — Constitution stratégique

Source du **Titre F2 — Constitution** (15 domaines, 18 lois, dictionnaire
bilingue, context map).

## Documents (reconstruction fidèle — R2-Corpus Lot 3)

> F2 n'a jamais eu de texte ratifié unique : son contenu était réparti dans les
> rapports de session, amendé par audits. Chaque chapitre est **reconstruit
> fidèlement** depuis ces sources (jamais depuis la mémoire), avec rapport de
> conformité. Découpage en sous-lots par chapitre.

| # | Document | Statut |
|---|----------|--------|
| 01 | [Domain Landscape Blueprint](01-domain-landscape.md) | **fait** (Lot 3a) — 15 domaines, état final |
| 02 | [Domain Context Map](02-context-map.md) | **fait** (Lot 3b) — 15 domaines, état final |
| 03 | [Ubiquitous Language • Responsibilities • Contracts](03-language-responsibilities-contracts.md) | **fait** (Lot 3c) |
| 04 | [Bilingual Dictionary](04-bilingual-dictionary.md) | **fait** (Lot 3d) — v1.1.0, propriétaire du vocabulaire |
| 05 | [Domain Rules • Invariants • Failure Modes](05-rules-invariants-failure-modes.md) | **fait** (Lot 3e) — lois 14-18 |
| 06 | [Architecture Constitution (le gel)](06-architecture-constitution.md) | **fait** (Lot 3f) — 9 Titres, articles P1-P18 |

Toute évolution passe par le **Titre VII**.
