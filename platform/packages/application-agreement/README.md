...
| 3 | 1C-5: Process Managers "uniquement ceux présents dans R2" | F4.2 names two canonical journeys: `ErasureProcess` (effacement — owned by the Compte journey, not Agreement) and **`NoShowSettlementProcess`** ("l'accord échu dont la rencontre ne s'ouvre jamais") — the Agreement-adjacent ratified PM | **RESOLVED 1C-5 by STOP**: the canon NAMES this PM but never SPECIFIES it — no subscription list, no emitted command, no positions of its own exist anywhere in R2 (F4.2 line 30 + "no-show : échéance + jonction" §4 are its ONLY two mentions); the no-show itself "n'est pas un fait (lecture composée)" (F2.2) and "reste une projection" (F3.2-A); `ExecuteSettlementOrder` is ratified "(via ACL)" inside the frozen chain `EncounterClosed → RevenueRecognized → … → SettlementOrder` (F3.3 §9); the Encounter/Settlement wire contracts are not materialized. Building it would AUTHOR a PM definition (subscriptions P-5, compensations P-8, payloads) — a Titre VII act, not a citation. The generic Séquence de Réaction WAS delivered (kernel `reaction/`); the PM awaits the CTO's Titre VII decision + the Consultation/Settlement published languages |
