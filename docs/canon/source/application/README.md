...
Source du **Titre F4 — Exécution** (les trois Séquences — Commande/Réaction/
Lecture —, Process Managers, Circulation, Infrastructure ; lois A / P / M / V / I).

## Documents (reconstruction fidèle — R2-Corpus Lot 5)

> Reconstruction fidèle depuis les sources de session ratifiées (jamais depuis
> la mémoire), avec rapport de conformité. Découpage F4 en trois lots :
> 5A (F4.1 + séquence de commande), 5B (F4.2 + F4.3), 5C (F4.4 + audit + clôture).
