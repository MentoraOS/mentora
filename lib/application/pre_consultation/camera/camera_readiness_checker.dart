...
import 'camera_readiness_provider.dart';

/// The camera checker — one responsibility: ask the provider, hand the
...
final class CameraReadinessChecker implements ConsultationReadinessChecker {
  const CameraReadinessChecker({required CameraReadinessProvider provider})
    : _provider = provider;

  final CameraReadinessProvider _provider;

  /// Matches ConsultationReadinessEngine.cameraCheckerId — asserted by
  /// test so the two can never drift apart.
  static const String checkerId = 'camera';
