 ...
 * (P-4). correlationId/causationId stay empty at write — the frozen port
 * retain(unit) cannot transport them ("corrélation portée quand elle
 * existe", F5.3 §2; SIGNALED).
 */
export class AgreementOutboxStore {
  constructor(private readonly messageIds: IdGenerator) {}

  async write(tx: Prisma.TransactionClient, facts: readonly AgreementDomainEvent[]): Promise<void> {
